...
A token is an encrypted data saved in local storage used to authenticate the user where a login attempt is not necessary every time the page is accessed.
...
Web servers are computers which hold the data necessary to show the webpages. They host these information to other computers.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

Create, read, update (put), delete
